...
namespace SIL.Machine.Morphology.HermitCrab.PhonologicalRules
{
	...
	public class AnalysisMetathesisRuleSpec : IPhonologicalPatternRuleSpec, IPhonologicalPatternSubruleSpec
	{
		...
		public AnalysisMetathesisRuleSpec(Pattern<Word, ShapeNode> pattern, string leftGroupName, string rightGroupName)
		{
			...
			foreach (PatternNode<Word, ShapeNode> node in pattern.Children.GetNodes(Direction.RightToLeft)
				.TakeWhile(n => !(n is Group<Word, ShapeNode>)).Reverse())
			{
				_pattern.Children.Add(node.Clone());
			}
			_pattern.Freeze();
		...
		void IPhonologicalPatternSubruleSpec.ApplyRhs(Match<Word, ShapeNode> targetMatch, Range<ShapeNode> range,
			VariableBindings varBindings)
		{
			...
			foreach (Tuple<ShapeNode, ShapeNode> tuple in targetMatch.Input.Shape.GetNodes(leftGroup.Range)
